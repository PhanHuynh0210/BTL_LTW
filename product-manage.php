<?php
    ...
    if(isset($_GET['enableQuery'])  && isset($_POST['submit']) && $_POST['submit'] == 'insert') {
        ...
        if($result1 && $result2) {
            ...
            echo "<script>
                alert('Thêm Điện Thoại mới có mã {$productId} thành công!');
    ...
    if(isset($_GET['enableQuery'])  && isset($_POST['submit']) && $_POST['submit'] == 'edit') {
        ...
        if($result) {
            ...
            echo "<script>
                alert('Sửa Điện Thoại có mã {$productId} thành công!');
                window.location.href = 'product-manage.php';
            </script>";
        }
     }
...
<script>
    eventForSideBar(3);
    setValueHeader("Điện Thoại");
</script>
<div class="container__product">
    <div class="container-product__header">
        <form class="container-product-header__search" autocomplete="off">
            <input name="product-search" type="text" placeholder="Tên, thương hiệu, màu, kiểu máy, Trạng Thái.." value="<?= $keyWord ?>">
        ...
        ?>
        <button class="container-product-header__insert" onclick="displayInsertModal('<?= $newProductId ?>');">Thêm Điện Thoại</button>
    </div>
    ...
    <table class="container-product__table">
        ...
        <tbody>
            ...
            if($result->num_rows > 0) {
                while ($row = mysqli_fetch_array($result)) {
                    ?>
                        <tr id="<?= $row['ProductID'] ?>">
                            ...
                            <td path = "<?= $row['ProductImg'] ?>"><img src="<?= $row['ProductImg'] ?>" alt="Ảnh Điện Thoại"></td>
                            <td><?= $row['Model'] ?></td>
                            <td><?= $row['Color'] ?></td>
                            <td><?= $row['Gender'] ?></td>
                ...
                ?>
                    <tr>
                        <td colspan="12" style="padding: 8px 0;">Không có Điện Thoại nào để hiển thị!</td>
                    </tr>
                <?php
            }
            mysqli_close($con);
